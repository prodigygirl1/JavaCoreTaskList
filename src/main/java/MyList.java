...
public class MyList<T> implements List<T>, AuthorHolder {
    ...
    public Iterator<T> iterator() {
        return new MyList.MyListIterator();
    }
    ...
    public ListIterator<T> listIterator(int i) {
        return new MyList.MyListIterator(i);
    }
    ...
    public List<T> subList(int i, int i1) {
        if (i < 0 || i1 > size() || i > i1)
            return null;
        MyList<T> resultList = new MyList<>();
        for (int j = i; i < i1; i++) {
    ...
    private class MyListIterator implements ListIterator<T> {
        ...
        public T next() {
            int i = this.cursor;
            if (i >= MyList.this.size()) {
                throw new NoSuchElementException();
            } else {
                T[] elementData = MyList.this.array;
        ...
        public void remove() {
            if (this.lastRet < 0) {
                ...
                try {
                    MyList.this.remove(this.lastRet);
        ...
        public void set(T t) {
            if (this.lastRet < 0) {
                ...
                try {
                    MyList.this.set(this.lastRet, t);
